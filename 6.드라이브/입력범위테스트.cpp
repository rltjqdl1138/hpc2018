...
int main(){
	...
	cin >> C >> E >> D >> n;
    if(C <1 || E < 1 || C > 500 || E > 500) assert(false);
    if(D < 1 || D > 10000) assert(false);
    if(n < 0 || n > 1000) assert(false);
	...
	for(int i=1; i<=n; ++i){
		cin >> price[i];
        if(price[i] < 1 || price[i] > 10000) assert(false);
    }
	price[0] = 123456;
